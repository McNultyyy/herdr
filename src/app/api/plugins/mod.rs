...
impl App {
    ...
    /// Resolve the plugin half of the workspace context menu, for both kinds
    /// of workspace at once.
    ///
    /// The client draws the menu, so it needs the list before a right-click, not
    /// after: caching one response covers every row in the sidebar. Which
    /// workspaces get plugin items is a config question the endpoint owns, so
    /// both resolutions are returned and the client picks by the row it clicked.
    pub(super) fn handle_plugin_workspace_menu(&mut self, id: String) -> String {
        if let Err(err) = self.refresh_installed_plugins() {
            return encode_error(id, "plugin_registry_load_failed", err.to_string());
        }
        let resolve =
            |plugins: menu::WorkspaceMenuPlugins| crate::api::schema::PluginWorkspaceMenuItems {
                items: plugins
                    .items
                    .into_iter()
                    .map(|item| crate::api::schema::PluginWorkspaceMenuItem {
                        plugin_id: item.plugin_id,
                        action_id: item.action_id,
                        title: item.title,
                    })
                    .collect(),
                hidden_builtins: plugins
                    .hidden_builtins
                    .into_iter()
                    .map(str::to_owned)
                    .collect(),
            };
        let git = resolve(self.state.workspace_menu_plugins(true));
        let plain = resolve(self.state.workspace_menu_plugins(false));
        encode_success(id, ResponseResult::PluginWorkspaceMenu { git, plain })
    }
