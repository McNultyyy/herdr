...
pub(super) enum ClientContextMenuAction {
    ...
    ClosePane,
    /// A plugin action, by index into the menu's own `plugin_items`.
    /// Resolved when the menu opened, so a registry change mid-menu cannot
    /// re-aim a click that was already drawn.
    Plugin(usize),
}
...
pub(super) struct ClientContextMenuOverlay {
    ...
    pub(super) y: u16,
    pub(super) highlighted: usize,
    /// Plugin actions listed after the built-in items. Resolved once, when
    /// the menu opens, so the list a click lands on is the one that was drawn.
    pub(super) plugin_items: Vec<crate::api::schema::PluginWorkspaceMenuItem>,
    /// Built-in labels left out because a listed plugin action replaces them.
    pub(super) hidden_builtins: Vec<String>,
}
...
/// The plugin half of the workspace context menu, as the endpoint last
/// resolved it. Cached because a right-click must draw immediately.
#[derive(Debug, Default, Clone)]
pub(super) struct ClientWorkspaceMenuPlugins {
    pub(super) git: crate::api::schema::PluginWorkspaceMenuItems,
    pub(super) plain: crate::api::schema::PluginWorkspaceMenuItems,
}
...
pub(crate) struct ClientShellState {
    ...
    /// Plugin workspace menu items, as the active endpoint last resolved them.
    /// `None` until the first response lands; the menu simply has no plugin
    /// section until then, and against an endpoint without the method it
    /// stays that way.
    pub(super) workspace_menu_plugins: Option<ClientWorkspaceMenuPlugins>,
    /// Whether this connection has already asked. Keeps a cold cache from
    /// re-requesting on every mouse event while the first reply is in flight.
    pub(super) workspace_menu_plugins_requested: bool,
...
impl ClientShellState {
    pub(crate) fn new(mut config: ClientShellConfig) -> Self {
        ...
        Self {
            ...
            next_request_id: 1,
            workspace_menu_plugins: None,
            workspace_menu_plugins_requested: false,
    ...
    pub(super) fn apply_active_snapshot(&mut self, mut snapshot: Box<ClientShellSnapshot>) {
        ...
        // A different machine or a rebooted server has its own plugin set;
        // the cached menu belongs to the previous one.
        if self.snapshot.is_none() || endpoint_boot_changed {
            self.workspace_menu_plugins = None;
            self.workspace_menu_plugins_requested = false;
        }
