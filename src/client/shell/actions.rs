    ...
    pub(crate) fn handle_endpoint_result(
        ...
        match pending.kind {
            ...
            PendingEndpointKind::PluginWorkspaceMenu => {
                if let Ok(crate::api::schema::ResponseResult::PluginWorkspaceMenu { git, plain }) =
                    result
                {
                    // An open menu keeps the list it was drawn with, so there is
                    // nothing to repaint; this lands for the next right-click.
                    self.workspace_menu_plugins = Some(ClientWorkspaceMenuPlugins { git, plain });
                }
                return (false, Vec::new());
            }
