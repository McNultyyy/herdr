...
impl ClientShellState {
    ...
    pub(super) fn handle_mouse(&mut self, mouse: MouseEvent, outcome: &mut ClientShellInput) {
        // Warm on the first mouse event of a connection, so the plugin section
        // is already resolved by the time a row is right-clicked.
        if !self.workspace_menu_plugins_requested {
            self.refresh_workspace_menu_plugins(outcome);
        }
        ...
        match mouse.kind {
            MouseEventKind::Down(MouseButton::Right) => {
                ...
                if let Some(workspace_id) = workspace_id {
                    ...
                    // For the next open: a plugin linked since the last refresh
                    // shows up without restarting the session.
                    self.refresh_workspace_menu_plugins(outcome);
                    outcome.repaint = true;
                    return;
                }
